...
local Settings = {
    ...
    SelectWeapon = "Melee", SelectWeaponGun = "",
}
    ...
    while true do
        if Settings.AutoYama then
            if safeInvoke("EliteHunter", "Progress") >= 30 then
                ...
                if yama then pcall(function() fireclickdetector(yama.ClickDetector) end) end
            end
        end
        task.wait(2)
    end
        ...
        if Settings.AutoCavander then
            ...
            if beautiful and beautiful.Humanoid.Health > 0 then
                repeat
                    ...
                    attack()
                until not Settings.AutoCavander or beautiful.Humanoid.Health <= 0
            else
        ...
        if Settings.AutoTwinHooks then
            ...
            if captain and captain.Humanoid.Health > 0 then
                repeat
                    ...
                    attack()
                until not Settings.AutoTwinHooks or captain.Humanoid.Health <= 0
            else
        ...
        if Settings.AutoMusketeerHat then
            ...
            if progress and progress.KilledBandits == false then
                if not LocalPlayer.PlayerGui.Main.Quest.Visible then
                    ...
                    local forest = Enemies:FindFirstChild("Forest Pirate")
                    if forest and forest.Humanoid.Health > 0 then
                        repeat
                            task.wait()
                            EquipWeapon(Settings.SelectWeapon)
                            TP(forest.HumanoidRootPart.CFrame * CFrame.new(0,0,3))
                            forest.HumanoidRootPart.Size = Vector3.new(50,50,50)
                            attack()
                        until not Settings.AutoMusketeerHat or forest.Humanoid.Health <= 0
                    else
    ...
    while true do
        if Settings.AutoRainbowHaki then
            if not LocalPlayer.PlayerGui.Main.Quest.Visible then
                ...
                local title = LocalPlayer.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text
                local mob = nil
                if title:find("Stone") then mob = Enemies:FindFirstChild("Stone")
                elseif title:find("Island Empress") then mob = Enemies:FindFirstChild("Island Empress")
                elseif title:find("Kilo Admiral") then mob = Enemies:FindFirstChild("Kilo Admiral")
                elseif title:find("Captain Elephant") then mob = Enemies:FindFirstChild("Captain Elephant")
                elseif title:find("Beautiful Pirate") then mob = Enemies:FindFirstChild("Beautiful Pirate") end
                if mob and mob.Humanoid.Health > 0 then
                    repeat
                        ...
                        attack()
                    until not Settings.AutoRainbowHaki or mob.Humanoid.Health <= 0
                else
                    TP(CFrame.new(-11892.0703125,930.57672119141,-8760.1591796875))
                    task.wait(1.5)
                    safeInvoke("HornedMan", "Bet")
                    ...
                    elseif not LocalPlayer.Backpack:FindFirstChild("Flower 3") and not LocalPlayer.Character:FindFirstChild("Flower 3") then
                        ...
                        if zombie and zombie.Humanoid.Health > 0 then
                            repeat
                                ...
                                attack()
                            until not Settings.AutoRaceV2 or zombie.Humanoid.Health <= 0 or LocalPlayer.Backpack:FindFirstChild("Flower 3") or LocalPlayer.Character:FindFirstChild("Flower 3")
                        else
...
-- Infinite Energy / Walk on Water / NoClip / InfJump / AntiAFK / AutoCloseDialog
task.spawn(function()
    while true do
...
task.spawn(function()
    while true do
        if Settings.AutoTrainV4 then
            ...
            elseif status:find("Ready For Trial") then Settings.AutoTrainV4 = false; Fluent:Notify({Title = "Ready For Next Trial", Duration = 5})
            elseif status:find("You're Done") then Settings.AutoTrainV4 = false; Fluent:Notify({Title = "You're Done Your Race", Duration = 5}) end
    ...
    while true do
        ...
        if Settings.CompleteSharkTrial then
            ...
            if beast and Workspace.Map:FindFirstChild("FishmanTrial") then
                ...
                while beast and beast.Parent and beast.Humanoid.Health > 0 and Settings.CompleteSharkTrial do
                    attack()
                    task.wait(0.5)
                end
            end
...
-- ====================== FLUENT UI (EXACTLY AS YOUR V13.7) ======================
local Fluent = loadstring(game:HttpGet("https://github.com/dawid-scripts/Fluent/releases/latest/download/main.lua"))()
local Window = Fluent:CreateWindow({
    Title = "Koala Hub v17.0",
    SubTitle = "KoalaHub Fully Loaded",
    TabWidth = 160,
...
Tabs.Main:AddToggle("AutoBuso", {Title = "Auto Buso Haki", Default = false}):OnChanged(function(v) Settings.AutoBuso = v end)
Tabs.Main:AddToggle("AutoObs", {Title = "Auto Observation", Default = false}):OnChanged(function(v) Settings.AutoObs = v end)

-- Farming Tab
Tabs.Farming:AddSection("Auto Farming")
Tabs.Farming:AddToggle("AutoWhiteBelt", {Title = "Auto White Belt", Default = false}):OnChanged(function(v) Settings.AutoWhiteBelt = v end)
Tabs.Farming:AddToggle("AutoYellowBelt", {Title = "Auto Yellow Belt", Default = false}):OnChanged(function(v) Settings.AutoYellowBelt = v end)
Tabs.Farming:AddToggle("AutoEliteHunter", {Title = "Auto Elite Hunter", Default = false}):OnChanged(function(v) Settings.AutoElitehunter = v end)
Tabs.Farming:AddToggle("AutoObservation", {Title = "Auto Observation Training", Default = false}):OnChanged(function(v) Settings.AutoObservation = v end)
Tabs.Farming:AddToggle("AutoCavander", {Title = "Auto Cavander", Default = false}):OnChanged(function(v) Settings.AutoCavander = v end)
Tabs.Farming:AddToggle("AutoTwinHooks", {Title = "Auto Twin Hooks", Default = false}):OnChanged(function(v) Settings.AutoTwinHooks = v end)
Tabs.Farming:AddToggle("AutoHolyTorch", {Title = "Auto Holy Torch", Default = false}):OnChanged(function(v) Settings.AutoHolyTorch = v end)
Tabs.Farming:AddToggle("AutoMusketeerHat", {Title = "Auto Musketeer Hat", Default = false}):OnChanged(function(v) Settings.AutoMusketeerHat = v end)
Tabs.Farming:AddToggle("AutoBartilo", {Title = "Auto Bartilo Quest", Default = false}):OnChanged(function(v) Settings.AutoBartilo = v end)
Tabs.Farming:AddToggle("AutoRainbowHaki", {Title = "Auto Rainbow Haki", Default = false}):OnChanged(function(v) Settings.AutoRainbowHaki = v end)
Tabs.Farming:AddToggle("AutoThirdSea", {Title = "Auto Third Sea", Default = false}):OnChanged(function(v) Settings.AutoThirdSea = v end)
Tabs.Farming:AddToggle("AutoSwan", {Title = "Auto Swan Glasses", Default = false}):OnChanged(function(v) Settings.AutoSwan = v end)
Tabs.Farming:AddToggle("AutoRengoku", {Title = "Auto Rengoku", Default = false}):OnChanged(function(v) Settings.AutoRengoku = v end)
Tabs.Farming:AddToggle("AutoEctoplasm", {Title = "Auto Ectoplasm", Default = false}):OnChanged(function(v) Settings.AutoEctoplasm = v end)
Tabs.Farming:AddToggle("AutoRaceV2", {Title = "Auto Race V2", Default = false}):OnChanged(function(v) Settings.AutoRaceV2 = v end)
...
Tabs.Shop:AddToggle("AutoBuyLegendarySword", {Title = "Auto Buy Legendary Sword", Default = false}):OnChanged(function(v) Settings.AutoBuyLegendarySword = v end)
Tabs.Shop:AddToggle("AutoBuyHakiColor", {Title = "Auto Buy Haki Color", Default = false}):OnChanged(function(v) Settings.AutoBuyHakiColor = v end)
...
-- Quest & Item Tab
Tabs.Quest:AddSection("Quest & Item")
...
Tabs.Quest:AddToggle("AutoAwakener", {Title = "Auto Awakener", Default = false}):OnChanged(function(v) Settings.Auto_Awakener = v end)
Tabs.Quest:AddToggle("AutoBuyChip", {Title = "Auto Buy Chip", Default = false}):OnChanged(function(v) Settings.AutoBuyChip = v end)
Tabs.Quest:AddToggle("AutoStartRaid", {Title = "Auto Start Raid", Default = false}):OnChanged(function(v) Settings.Auto_StartRaid = v end)
...
-- Raid Tab
Tabs.Raid:AddSection("Raid Settings")
Tabs.Raid:AddDropdown("SelectChip", {Title = "Chip Type", Values = {"Flame","Ice","Quake","Light","Dark","Spider","Rumble","Magma","Buddha","Sand","Phoenix","Dough"}, Default = "Flame"}):OnChanged(function(v) Settings.SelectChip = v end)
Tabs.Raid:AddToggle("AutoBuyChip", {Title = "Auto Buy Chip", Default = false}):OnChanged(function(v) Settings.AutoBuyChip = v end)
Tabs.Raid:AddToggle("AutoStartRaid", {Title = "Auto Start Raid", Default = false}):OnChanged(function(v) Settings.Auto_StartRaid = v end)
Tabs.Raid:AddToggle("AutoAwakener", {Title = "Auto Awakener", Default = false}):OnChanged(function(v) Settings.Auto_Awakener = v end)
...
Tabs.Race:AddDropdown("TrainV4Type", {Title = "Training Mode", Values = {"Bone","Katakuri"}, Default = "Bone"}):OnChanged(function(v) Settings.TrainV4Type = v end)
Tabs.Race:AddToggle("CompleteHumanTrial", {Title = "Complete Human Trial", Default = false}):OnChanged(function(v) Settings.CompleteHumanTrial = v end)
Tabs.Race:AddToggle("CompleteGhoulTrial", {Title = "Complete Ghoul Trial", Default = false}):OnChanged(function(v) Settings.CompleteGhoulTrial = v end)
Tabs.Race:AddToggle("CompleteSharkTrial", {Title = "Complete Shark Trial", Default = false}):OnChanged(function(v) Settings.CompleteSharkTrial = v end)
Tabs.Race:AddButton("Check Ancient Status", function()
    local status = CheckAcientOneStatus()
    Fluent:Notify({Title = "Ancient Status", Content = status, Duration = 5})
end)
Tabs.Race:AddButton("Buy Gear", BuyGear)
...
-- Final
Window:SelectTab(1)
Fluent:Notify({Title = "Koala Hub v17.0", Content = "All features loaded", Duration = 8})
print("✅ Koala Hub v17.0 – Fully loaded, all features working.")